/**
 * Independent QA Judge — evidence integrity challenge (Phase 7).
 * Reads only filesystem evidence + live frozen candidate; no Builder chat.
 *
 *   node QA/scripts/phase7-judge.challenge.mjs \
 *     --candidate cand-… \
 *     --cert-record Evidence/phase-7/…/certification-run-record.json \
 *     --qa-findings QA/findings/PHASE_7_QA_FINDINGS.md \
 *     --out QA/findings/PHASE_7_JUDGE_CHALLENGE.md
 */
...
async function main() {
  ...
  const out = arg('out', 'QA/findings/PHASE_7_JUDGE_CHALLENGE.md');
  ...
  if (live.candidateId !== candidate) {
    ...
    notes.push(`Live frozen origin serves ${candidate} (${live.runtimeMode}, ${live.publicSurface})`);
  }

  const pack = await fetch(`${arena}/api/release/gold-master`).then((r) => r.json());
  if (pack.candidateId !== candidate) {
    failures.push(`Gold Master package candidate ${pack.candidateId} ≠ claimed ${candidate}`);
  }
  if (pack.launchProduction === 'PRODUCTION_DEPLOYED' || pack.hostedSmoke === 'PASSED') {
    failures.push('Deploy fiction: package claims Launch Production deploy or hosted smoke without this host’s credentials');
  } else {
    notes.push(`Gold Master package launchProduction=${pack.launchProduction} authorization=${pack.productOwnerAuthorization}`);
  }
  if (!Array.isArray(pack.strippedFromHostedArtifacts) || !pack.strippedFromHostedArtifacts.includes('development_identity_mint')) {
    failures.push('Harness leak: Gold Master package does not list development_identity_mint as stripped');
  } else {
    notes.push('Gold Master package lists development identity mint as stripped from hosted artifacts');
  }
  ...
  if (/PRODUCTION_DEPLOYED|Safari certified|VoiceOver passed/i.test(findings)) {
    failures.push('QA findings appear to claim hosted deploy or real Safari/AT certification');
  }
  notes.push('QA findings file present and scanned for candidate binding and deploy fiction');
...
# Phase 7 — Independent QA Judge Challenge
