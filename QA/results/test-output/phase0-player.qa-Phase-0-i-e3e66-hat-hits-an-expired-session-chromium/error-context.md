...
- Name: phase0-player.qa.spec.ts >> Phase 0 independent QA — failure explanation on the page >> QA-S16 the same invisibility affects a read that hits an expired session
- Location: scripts/phase0-player.qa.spec.ts:842:7

# Error details

```
Error: the page should say why it stopped showing the records

expect(received).toMatch(expected)

Expected pattern: /enter the local arena before|session expired/i
...
- generic [active] [ref=e1]:
  ...
  - generic [ref=e4]:
    ...
    - status [ref=e21]: Recorded sequence 1.
```
